...
interface FeedPost {
  ...
  userEngagement: {
    ...
    hasSaved: boolean;
    hasShared?: boolean;
    // WHICH reaction the server recorded. `null` beside `hasLiked: true` is a
    // pre-migration untyped like — displayed as Happy, never written back as
    // HAPPY.
...
export default function CommunityDetailPage() {
      ...
      prev.map((p) =>
        p.id === postId
          ? {
              ...p,
              engagementCounts: {
                // The spread is load-bearing: this literal REPLACES the whole
                // object, so without it the first optimistic tap strips
                // happy/hopeful/sad off the post and the reaction cluster
                // collapses for the rest of the session.
                ...p.engagementCounts,
                // A per-kind delta only applies to a bucket the server actually
                // reported. Creating one from undefined would invent a measured
                // breakdown out of a single optimistic tap.
              ...
              userEngagement: {
                ...
                // WHICH reaction, carried optimistically alongside hasLiked.
                // Tested against `undefined`, not truthiness: `null` is a REAL
                // value here (the viewer cleared their reaction), and treating
                // it as "no opinion" would leave the old kind selected.
   ...
   * they looked at another tab. `localPosts` is this component's own state, two
   * levels above that switch, and a tab change is a same-route `?tab=` soft
   * navigation which does not remount it.
   *
  ...
  const handleReact = async (
    ...
    delta: number,
    // The RAW prior `myReaction`, NOT the card's derived selection. They differ
    // for a pre-migration untyped like, where the derived value reports HAPPY
    // for display — rolling back with it would claim a reaction type the row
    // never had, turning a refused mutation into a silent data change.
    previousReaction: ReactionKind | null,
  ) => {
    const adding = op === 'add';
    // A SWITCH (Happy→Sad) sends delta 0 — the server updates the LIKE row in
    // place, so the total must not move. What hasLiked was BEFORE this change:
    //   remove           -> was reacted
    ...
    // ONE update. `myReaction` and the per-kind counts travel WITH `hasLiked`,
    // forwards and on rollback: splitting them tells the card "you reacted"
    // while leaving it to guess which (it guesses Happy — the "first tap always
    // selects Happy" bug), or leaves the old kind in the breakdown beside the
    // new one with a doubled total.
  ...
  const handleLeaveConfirm = async () => {
    try {
      const result = await leaveCommunity({ variables: { communityId } });
