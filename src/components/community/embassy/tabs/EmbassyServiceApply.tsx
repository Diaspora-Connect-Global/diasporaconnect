...
export function EmbassyServiceApply({ serviceId, community, profile }: EmbassyServiceApplyProps) {
  ...
  async function runSubmit() {
    ...
    try {
      ...
      const submitOutcome = readMutationOutcome(submit, d => d.submitServiceRequest);
      if (!submitOutcome.ok) {
        throw new Error(refusalMessageKey(submitOutcome.message, 'requests.errors'));
      }
      ...
      for (const [fieldKey, file] of Object.entries(files)) {
        ...
        const uploadOutcome = readMutationOutcome(u, d => d.requestServiceRequestDocumentUploadUrl);
        if (!uploadOutcome.ok) {
          throw new Error(refusalMessageKey(uploadOutcome.message, 'requests.errors'));
        }
        ...
        const addOutcome = await addServiceRequestDocument({
          variables: { requestId: res.id, documentId: upload.documentId },
        });
        const docOutcome = readMutationOutcome(addOutcome, d => d.addServiceRequestDocument);
        if (!docOutcome.ok) {
          throw new Error(refusalMessageKey(docOutcome.message, 'requests.errors'));
        }
      }
