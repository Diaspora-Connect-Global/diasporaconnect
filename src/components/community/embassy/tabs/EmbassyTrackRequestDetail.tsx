...
function SupplyInfoSection({ request, formFields, onSupplied }: SupplyInfoSectionProps) {
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (file) {
        const urlResult = await requestDocUploadUrl({
          variables: {
        ...
        const upload = urlResult.data?.requestServiceRequestDocumentUploadUrl;
        if (upload) {
          ...
          const docResult = await addServiceRequestDocument({
