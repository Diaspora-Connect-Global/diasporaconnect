...
/**
 * INDEPENDENT COPY of the `/home` feed, serving `/[locale]/home2`.
 *
 * This is a deliberate duplicate, not a refactor. `/en` and `/home` render
 * `(home)/home/page.tsx`; this file is a snapshot of it taken 2026-09-04 so
 * `/home2` can be changed without touching the live home feed.
 *
 * BECAUSE IT IS A COPY IT WILL DRIFT. A fix landed in the original does not
 * reach here, and vice versa. If the two are meant to stay identical, delete
 * this file and point `Home2Feed` back at the original instead — sharing is
 * cheaper than syncing. If they are meant to diverge, that is the whole point,
 * and this comment is the record of where it came from.
 */
...
// FeedCard2 React.memo (a fresh closure per render would defeat it).
...
// which preserves the FeedCard2 React.memo (a fresh closure per render
// would defeat it).
...
export default function HomeFeed2() {
  ...
  const handleSave = useCallback(async (postId: string, saved: boolean) => {
    ...
    try {
      ...
      const outcome = readMutationOutcome(result, (d) =>
        saved ? d?.addEngagement : d?.removeEngagement,
      );
      if (!outcome.ok) {
        rollback();
        toast.error(tRoot(refusalMessageKey(outcome.message, 'feed.errors')));
  ...
  const handleShare = useCallback(async (postId: string) => {
    ...
    const rollback = () => updatePostCounts(postId, { shares: -1 });
    try {
      const result = await addEngagement({
        variables: { input: { postId, engagementType: 'SHARE' } },
      });
      const outcome = readMutationOutcome(result, (d) => d?.addEngagement);
      if (!outcome.ok) {
        rollback();
        toast.error(tRoot(refusalMessageKey(outcome.message, 'feed.errors')));
      }
  ...
  }, [addEngagement, updatePostCounts, tRoot]);
  ...
  // Stable handlers so memoised FeedCard2 instances don't see a
