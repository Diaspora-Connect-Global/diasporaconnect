'use client';
/* =====================================================================
 *  FeedCard2 — SNAPSHOT COPY of `@/components/cards/FeedCardWithReply`,
 *  taken for the /home2 feed clone.
 *
 *  This file was copied verbatim and then diverged. It is NOT kept in
 *  sync with the original: fixes landing in FeedCardWithReply do not
 *  reach here, and changes here must never be assumed to exist there.
 *  The whole point of /home2 is that it can move without touching the
 *  live `/en` and `/home` feeds, which still render the shared card.
 *
 *  Divergence so far:
...
interface FeedCardProps {
    ...
    comments: number;
    shares: number;
    /** REAL: `engagementCounts.saves`. Added for /home2 — the shared card
     *  never displayed it, so the original has no such prop. */
    saves?: number;
    /**
     * Per-reaction counts. OPTIONAL and undefined today — the API returns
     * only `likes`. Undefined ⇒ no breakdown UI renders at all. Pass it
     * through the day post-feed-service returns it and the panel appears.
     */
    reactionBreakdown?: ReactionBreakdown;
    /**
     * The reaction the SERVER recorded. Does not exist yet; wire it up when
     * post-feed-service ships reaction types and every pick becomes durable.
     */
...
function FeedCard2Inner({
    ...
    /**
     * The reaction picked in THIS session.
     *
     * HAPPY is the existing Like, so it is server-backed and lives in
     * `isLiked`; this only carries HOPEFUL / SAD, which post-feed-service
    ...
    const selectedReaction = readSelectedReaction(
        { hasLiked: isLiked, reaction: serverReaction },
        sessionReaction,
    );
    ...
    // HAPPY, because Happy IS the Like. The surfaces can never disagree.
    const handleLike = () => {
    ...
    return (
        <>
            ...
            <div className="relative w-full bg-surface-default border border-border-subtle rounded-lg p-[1rem] flex flex-col my-[0.5rem]">
                ...
                {/* Grouped reaction control + every interaction count in one row.
                    Always rendered (unlike the shared card's bar, which hid at
...
const FeedCard2 = memo(FeedCard2Inner);
FeedCard2.displayName = 'FeedCard2';
